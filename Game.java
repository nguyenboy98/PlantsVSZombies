...
public static final int WIDTH = 1400, HEIGHT = 600;// WIDTH / 12 * 9;
       ...
       private void render(){
		...
		public void mousePressed(MouseEvent e) {
			...
			int y= e.getY();
			if(State==STATE.END) {
				if(x>450&&x<950&&y>500&&y<590) {State=STATE.GAME;zombiescount=1;k3=7;}
				
				if(x>0&&x<100&&y>0&&y<100) {System.exit(1);}
			}
			if(State==STATE.WIN) {
				if(x>700&&x<800&&y>450&&y<550) {State=STATE.MENU;levl=1;countplants=1;zombiescount=1;}
	...
	public void level() {
		if(countplants==0) {
			...
			JOptionPane.showMessageDialog(null, "welcome to satage "+levl);
			gamestart();}}
